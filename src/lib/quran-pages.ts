import { SURAHS, arabicDigits } from "@/lib/quran-surahs";
...
const CUMULATIVE = SURAHS.reduce<number[]>((acc, surah) => {
  acc.push((acc[acc.length - 1] ?? 0) + surah.ayahs);
  return acc;
}, []);

const TOTAL_AYAHS = CUMULATIVE[CUMULATIVE.length - 1] ?? 6236;

const globalIndex = (surahNumber: number, ayah: number) =>
  (CUMULATIVE[surahNumber - 2] ?? 0) + ayah;

const PAGE_BOUNDS = PAGE_STARTS.map(([surah, ayah], index) => {
  const start = globalIndex(surah ?? 1, ayah ?? 1);
  const nextRef = PAGE_STARTS[index + 1];
  const end = nextRef ? globalIndex(nextRef[0], nextRef[1]) - 1 : TOTAL_AYAHS;
  return { start, end, length: Math.max(1, end - start + 1) };
});

/** Fractional number of mushaf pages covered by an ayah range. */
export function pagesForRange(surahNumber: number, fromAyah: number, toAyah: number) {
  const surah = SURAHS[surahNumber - 1];
  if (!surah) return 0;
  const from = Math.min(Math.max(fromAyah, 1), surah.ayahs);
  const to = Math.min(Math.max(toAyah, from), surah.ayahs);
  const start = globalIndex(surahNumber, from);
  const end = globalIndex(surahNumber, to);
  let pages = 0;
  for (const page of PAGE_BOUNDS) {
    if (page.end < start) continue;
    if (page.start > end) break;
    const overlap = Math.min(page.end, end) - Math.max(page.start, start) + 1;
    pages += overlap / page.length;
  }
  return pages;
}
