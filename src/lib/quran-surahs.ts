...
export function ayahCountText(count: number) {
  ...
  if (count <= 10) return `${arabicDigits(count)} آيات`;
  return `${arabicDigits(count)} آية`;
}

export function formatMemorizationRange(surahIndex: number, fromAyah: number, toAyah: number) {
  ...
  if (from === 1 && to === surah.ayahs) return `سورة ${surah.name} كاملة (${ayahCountText(count)})`;
  if (from === to) return `سورة ${surah.name}: الآية ${arabicDigits(from)} (${ayahCountText(count)})`;
  return `سورة ${surah.name}: من الآية ${arabicDigits(from)} إلى الآية ${arabicDigits(to)} (${ayahCountText(count)})`;
}
